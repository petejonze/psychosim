classdef AdaptiveTrack < handle
    ...
    methods (Access = 'public')
        ...
        function [delta, stage] = getDelta(obj)
            ...
            if isempty(stage) % defensive
                error('no stage has been set???');
            end
        ...
        function [nRightInARow, nWrongInARow] = getNInARow(obj)
            lastWasWrong = obj.wasCorrectHistory(end) == 0;
            nWrongInARow = lastWasWrong;
            nRightInARow = ~lastWasWrong;
            ...
            for i = length(dx):-1:1
                if dx(i) ~= 0
                    ...
                    if lastWasWrong
                        nWrongInARow = nWrongInARow + 1;
                    else
                        nRightInARow = nRightInARow + 1;
                    end
                end
            end
        end
